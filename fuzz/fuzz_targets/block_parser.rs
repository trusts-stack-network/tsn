...
//!
//! Teste la robustesse du parser de blocs face a des entrees malformedes,
//! corrompues ou malveillantes. Objectif : aucun panic, crash ou comportement
//! indefini, same avec des data adversaires.
//!
//! THREAT MODEL:
...
/// Point d'entree du fuzzer pour le parsing de blocs
fuzz_target!(|data: &[u8]| {
    ...
    panic::set_hook(Box::new(|_| {
        // Ne rien faire - on veut juste detect le panic
    }));
    ...
    match result {
        Ok(_) => {
            // Parsing reussi ou echec gracieux - OK
        },
        Err(_) => {
            // Panic detecte - c'est un bug !
            eprintln!("PANIC detecte lors du parsing de bloc avec {} bytes", data.len());
            eprintln!("Data: {:?}", data);
...
fn fuzz_block_parsing(data: &[u8]) {
    // Test 1: Tentative de deserialization directe
    if data.len() >= 8 {
        let _ = try_deserialize_block(data);
    }
    
    // Test 2: Construction de bloc avec des data fuzzees
    if data.len() >= 32 {
        let _ = try_construct_block_from_fuzz_data(data);
    }
    
    // Test 3: Validation de blocs construits avec des data fuzzees
    if data.len() >= 64 {
        let _ = try_validate_fuzzed_block(data);
    }
    
    // Test 4: Calculs de hash avec des data corrompues
    if data.len() >= 16 {
        let _ = try_hash_calculations(data);
    }
    
    // Test 5: Merkle tree avec transactions fuzzees
    if data.len() >= 100 {
        let _ = try_merkle_tree_fuzzing(data);
    }
}

/// Tentative de deserialization de bloc a partir of data brutes
fn try_deserialize_block(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    // En vrai, on usesrait serde ou bincode pour deserialiser
    // Ici on simule en extrayant des champs depuis les bytes bruts
    
    if data.len() < 80 { // Taille minimale d'un bloc
        return Err("Data trop courtes".into());
    }
    
    // Extraire les champs avec verification de bounds
    ...
    // Check that le bloc peut calculer son hash sans paniquer
    let _hash = block.calculate_hash();
    
    Ok(())
}

/// Construction de bloc avec des parameters fuzzes
fn try_construct_block_from_fuzz_data(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let mut offset = 0;
    
    // Extraire height avec protection contre les valeurs extreme
    let height = if data.len() >= offset + 8 {
        let raw_height = u64::from_le_bytes(data[offset..offset+8].try_into().unwrap());
        offset += 8;
        // Limiter a une valeur raisonnable pour avoid les DoS
    ...
    let block = Block::new(
        ...
        parent_hash,
        Vec::new(),
        [0u8; 32], // merkle_root sera calcule
        timestamp,
        nonce,
    );
    
    // Tester les operations sur le bloc
    let _hash = block.calculate_hash();
    let _merkle_root = block.calculate_merkle_root();
    
    // Check the serialization (simulation)
    let _serialized = format!("{:?}", block);
    
    Ok(())
}

/// Validation de blocs avec des data fuzzees
fn try_validate_fuzzed_block(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Construire un bloc a partir des data fuzzees
    let block = match try_construct_block_from_fuzz_data(data) {
        ...
        Err(_) => return Ok(()), // Failure de construction - pas grave
    };
    ...
    // Ces validations ne doivent jamais paniquer, same avec des data invalids
...
/// Test des calculs de hash avec des data corrompues
fn try_hash_calculations(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    // Tester le calcul de hash avec differentes tailles of data
    for chunk_size in [1, 2, 4, 8, 16, 32, 64].iter() {
        if data.len() >= *chunk_size {
            let chunk = &data[..*chunk_size];
            
            // Simuler le calcul de hash (en vrai on usesrait SHA256 ou Blake3)
    ...
    // Tester avec des data de taille variable
...
/// Test du merkle tree avec des transactions fuzzees
fn try_merkle_tree_fuzzing(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    use tsn::core::Transaction;
    use tsn::crypto::keys::KeyPair;
    
    // Generate des transactions a partir des data fuzzees
    let mut transactions = Vec::new();
    let mut offset = 0;
    
    // Limiter le nombre de transactions pour avoid les DoS
    let max_transactions = 100;
    let mut tx_count = 0;
    
    while offset + 64 <= data.len() && tx_count < max_transactions {
        // Extraire les data pour une transaction
        let sender_kp = KeyPair::generate(); // En vrai, on extrairait des data
        ...
        // Avancer l'offset pour avoid les loops infinies
...
/// Tests de cas limites specifiques
#[cfg(test)]
mod edge_case_tests {
    use super::*;
