//! Fuzzer complete pour la deserialization des transactions TSN
//!
//! Couvre:
...
//! - Validation des invariants de security
//!
//! THREAT MODEL:
//! - Double-spend via nullifiers dupliques
//! - Overflow de montants
//! - Transactions sans spend ni output (sauf coinbase)
//! - Attaques DoS via transactions surdimensionnees
...
fn prepare_data(data: &[u8], attack: AttackVector) -> Vec<u8> {
    ...
    match attack {
        ...
        AttackVector::OversizedData => {
            // Ne rien faire, les data sont already la
        }
...
fn fuzz_shielded_v1(data: &[u8]) {
    ...
    // Test 2: Validation des invariants si deserialization reussie
    if let Ok(tx) = result {
        // Invariant: une transaction doit avoir at least un spend OU un output
        // (sauf coinbase qui est traite separement)
        if tx.spends.is_empty() && tx.outputs.is_empty() {
            // Transaction vide - potentiellement invalid selon les regles
            // Mais ne doit pas causer de panic
        }
        
        // Invariant: checksr les nullifiers uniques (protection double-spend)
        let mut nullifiers = std::collections::HashSet::new();
        for spend in &tx.spends {
            let nf_bytes = spend.nullifier.to_bytes();
            if !nullifiers.insert(nf_bytes) {
                // Nullifier duplique detecte - c'est une double-spend potentielle
                // Le code ne doit pas paniquer, juste rejeter la transaction
            }
        }
        
        // Invariant: checksr les montants
        let mut total_output_value: u64 = 0;
        for output in &tx.outputs {
            // Les montants sont dans les encrypted notes, on ne peut pas les checksr directement
            // Mais on peut checksr la structure
        }
        ...
        // Test 4: Acces aux nullifiers
        let _nullifiers = tx.nullifiers();
        
        // Test 5: Acces aux commitments
        let _commitments = tx.note_commitments();
        
        // Test 6: Verification de la binding signature
...
fn fuzz_shielded_v2(data: &[u8]) {
    ...
    // Test 2: Validation si reussie
    if let Ok(tx) = result {
        // Check thes invariants similaires a V1
        if tx.spends.is_empty() && tx.outputs.is_empty() {
            // Transaction vide
        }
        
        // Check thes nullifiers uniques
        let mut nullifiers = std::collections::HashSet::new();
        for spend in &tx.spends {
...
fn fuzz_coinbase(data: &[u8]) {
    ...
    if let Ok(tx) = result {
        ...
        // (elle creates des nouveaux coins)
        
        // Check thes commitments
...
fn fuzz_spend_description(data: &[u8]) {
    ...
    if let Ok(spend) = result {
        // Invariant: anchor doit be 32 bytes
        // (already verified par le type)
...
fn fuzz_binding_signature(data: &[u8]) {
    ...
    if let Ok(sig) = result {
        ...
        // Check that ce n'est pas une signature vide
        if sig.as_bytes().is_empty() {
            // Signature vide - invalid mais ne doit pas paniquer
        }
    }
