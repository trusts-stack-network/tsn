...
fuzz_target!(|data: &[u8]| {
    // Fuzzer pour les commitments et notes malformeds
    if let Ok(commitment) = Commitment::from_bytes(data) {
        // Tester que les commitments malformeds ne causent pas de panics
        let _ = commitment.to_bytes();
        
        // Tester l'ouverture de commitments avec des data invalids
        let random_bytes: [u8; 32] = rand::random();
        let _ = commitment.verify_opening(&random_bytes.into());
    }
    
    if let Ok(note) = Note::from_bytes(data) {
        // Check that les notes malformedes sont correctement gerees
