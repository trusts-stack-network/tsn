...
fuzz_target!(|input: FuzzHeaderInput| {
    ...
    if let Ok(header) = bincode::deserialize::<BlockHeader>(&input.data) {
        ...
        if input.test_timestamp_attacks {
            ...
            // SECURITE: Timestamp old (avant Bitcoin genesis)
        ...
        if input.test_hash_validation {
            // SECURITE CRITIQUE: Hashes ne doivent jamais be nuls
            if header.prev_hash == [0u8; BLOCK_HASH_SIZE] {
                // Exception: bloc genesis peut avoir prev_hash nul
                if header.timestamp > 1230768000 + 86400 { // Plus de 24h after genesis
            ...
            // SECURITE: Check that les roots sont differents (sauf cas speciaux)
            ...
            // SECURITE: Hash du header doit be deterministic
        ...
        if input.test_pow_bypass {
            // SECURITE CRITIQUE: Le nonce doit be coherent avec la difficulty
            if header.difficulty > 10 {
                // Pour des difficulties elevees, checksr que ce n'est pas un faux PoW
                let hash = header.hash();
        ...
        // Test 7: Contraintes de consistency globale
        // SECURITE: Check the consistency entre tous les champs
        let computed_hash = header.hash();
        
        // Le hash ne doit pas be entierement nul (probabilite astronomiquement faible)
        if computed_hash == [0u8; BLOCK_HASH_SIZE] {
            panic!("Computed hash is all zeros - astronomical probability or attack");
        }
        
        // Le hash ne doit pas be entierement 0xFF (pattern suspect)
        if computed_hash == [0xFFu8; BLOCK_HASH_SIZE] {
            panic!("Computed hash is all 0xFF - suspicious pattern");
        }
    }
    
    // Test 8: Attaque par header malformed/tronque
    if input.data.len() > 4 && input.data.len() < 200 {
        // Headers tronques ne doivent jamais causer de panic
        let _ = bincode::deserialize::<BlockHeader>(&input.data);
    }
    
    // Test 9: Attaque par header surdimensionne (DoS)
    if input.data.len() > 100_000 {
        // DoS protection: parsing doit fail rapidement
