...
/// FUZZER: Deserialization TsnHandshake
/// 
/// Cible: Check the robustesse du protocole de handshake TSN
...
fuzz_target!(|input: HandshakeFuzzInput| {
    // Executer l'attaque sans panic - retourner un bool pour libfuzzer
    let result = execute_handshake_attack_safely(&input);
    
    // libfuzzer detecte les crashes via les panic ou les process exits
    // Nous retournons simplement pour indiquer que le test a termine
    // Les vulnerabilitys sont loguees mais ne causent pas de crash
    match result {
...
/// Attaque standard: deserialization simple avec validation
fn attack_standard_deserialize(input: &HandshakeFuzzInput) -> FuzzResult {
    ...
    // Check thes performances - retourner un result plutot que panic
...
/// Tente de forcer l'utilisation d'une version oldne/vulnerable
fn attack_version_downgrade(_input: &HandshakeFuzzInput) -> FuzzResult {
    let malicious_versions = [
        0u32,           // Version 0 (invalid)
        1,              // Tres oldne
        u32::MAX,       // Version future impossible
        PROTOCOL_VERSION.saturating_sub(1), // Version precedente
    ...
    for version in malicious_versions {
        ...
        if let Ok(deserialized) = bincode::deserialize::<TsnHandshake>(&serialized) {
            // Check that la version n'est pas dans le futur lointain
...
/// Tente de create une liste de capabilities massive
fn attack_capabilities_overflow(_input: &HandshakeFuzzInput) -> FuzzResult {
    // Create a liste massive de capabilities
    ...
    // Check that le parsing d'une liste massive ne cause pas de DoS
...
fn attack_timestamp_manipulation(_input: &HandshakeFuzzInput) -> FuzzResult {
    ...
    for timestamp in malicious_timestamps {
        ...
        if let Ok(serialized) = bincode::serialize(&handshake) {
            if let Ok(deserialized) = bincode::deserialize::<TsnHandshake>(&serialized) {
                // Le timestamp doit be preserve exactement
...
fn attack_peer_id_manipulation(_input: &HandshakeFuzzInput) -> FuzzResult {
    ...
    for size in peer_id_sizes {
        ...
        // Check that les PeerId massifs ne causent pas de DoS
...
fn attack_extra_fields(input: &HandshakeFuzzInput) -> FuzzResult {
    // Ajouter des data after un handshake valide
    ...
    if let Ok(mut serialized) = bincode::serialize(&valid_handshake) {
        // Ajouter des octets supplementaires
        serialized.extend_from_slice(&input.raw_data);
        
        // La deserialization devrait soit reussir (ignorant les data extra)
        // soit fail proprement
        let _: Result<TsnHandshake, _> = bincode::deserialize(&serialized);
    }
...
fn attack_null_bytes(input: &HandshakeFuzzInput) -> FuzzResult {
    ...
    // Inserer des null bytes a intervalles reguliers
...
fn attack_bit_flipping(input: &HandshakeFuzzInput) -> FuzzResult {
    ...
    // Flipper des bits a intervalles reguliers
