...
fuzz_target!(|data: &[u8]| {
    ...
    // Fuzzer pour les entrees de commitment
    let note = Note::from_bytes(data);
    
    // Test que le hash est deterministic
    let commit1 = compute_commitment(&note, b"domain1");
    let commit2 = compute_commitment(&note, b"domain1");
    assert_eq!(commit1, commit2, "Commitment non-deterministic");
    
    // Test de resistance aux collisions
    let commit3 = compute_commitment(&note, b"domain2");
    assert_ne!(commit1, commit3, "Collision de domaine detectee");
    
    // Check that le commitment ne fuite pas d'information
    ...
    assert!(bit_diffs > 100 && bit_diffs < 150, 
            "Faible diffusion detectee: {} bits changes", bit_diffs);
});