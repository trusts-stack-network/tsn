...
//!
//! Ce fuzzer teste la robustesse de l'implementation Poseidon contre
//! des inputs malformeds, extreme ou malveillants.
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(input) = PoseidonFuzzInput::arbitrary(&mut u) {
        ...
        // Cette operation ne doit JAMAIS paniquer
        let result = std::panic::catch_unwind(|| {
            poseidon_hash(input.domain, &field_inputs)
        });
        
        if result.is_err() {
            panic!("poseidon_hash panique avec domain={}, inputs.len()={}", 
                   input.domain, field_inputs.len());
        }
        
        // Si on a at least 2 inputs, tester poseidon_hash_2
        if field_inputs.len() >= 2 {
            ...
            if result2.is_err() {
                panic!("poseidon_hash_2 panique avec domain={}", input.domain);
            }
        }
    }
    ...
    if data.len() >= 32 {
        ...
        let result = std::panic::catch_unwind(|| {
            ...
            // Check that la conversion est coherente
            // Note: Il peut y avoir une difference due a la reduction modulo le prime du field
            let field2 = bytes32_to_field(&back_to_bytes);
            assert_eq!(field, field2, "Conversion field incoherente");
        });
        
        if result.is_err() {
            panic!("Conversion bytes32_to_field/field_to_bytes32 paniquee");
        }
    }
    
    // Test 3: Fuzzer avec des domaines extreme
    if let Ok(domain) = u64::arbitrary(&mut u) {
        if let Ok(field_elem) = FieldElement::arbitrary(&mut u) {
            ...
            if result.is_err() {
                panic!("poseidon_hash panique avec domaine extreme: {}", domain);
            }
        }
    }
    ...
    if data.len() >= 100 {
        let num_inputs = (data[0] as usize % 50) + 1; // 1 a 50 inputs
        ...
        if !inputs.is_empty() {
            ...
            if result.is_err() {
                panic!("poseidon_hash panique avec {} inputs", inputs.len());
            }
        }
    }
});