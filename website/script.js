...
// Dynamic Roadmap functionality
async function loadDynamicRoadmap() {
    try {
        const response = await fetch('/api/roadmap');
        const data = await response.json();
        updateRoadmapDisplay(data);
        ...
        console.warn('Failed to load dynamic roadmap:', error);
...
function updateRoadmapDisplay(roadmapData) {
    ...
    // Create timeline items from API data
    roadmapData.milestones.forEach(milestone => {
        ...
        timelineItem.dataset.progress = milestone.progress_pct;
        timelineItem.dataset.lastUpdated = roadmapData.last_updated;

        timelineItem.innerHTML = `
            <div class="timeline-date">${milestone.quarter}</div>
            <div class="timeline-content">
                <h3>${milestone.name} <span class="progress-badge">${Math.round(milestone.progress_pct)}%</span></h3>
                <p>${milestone.description}</p>
                <div class="metrics-summary">
                    ${formatMetrics(milestone.metrics)}
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${milestone.progress_pct}%"></div>
                </div>
            </div>
        `;
    ...
    updateNetworkHealthDisplay(roadmapData.network_health);

    // Add last updated timestamp
    const lastUpdated = new Date(roadmapData.last_updated * 1000);
    const statusIndicator = document.querySelector('.roadmap-status') || createStatusIndicator();
    statusIndicator.innerHTML = `
        <span class="status-dot ${getNetworkStatus(roadmapData.network_health)}"></span>
...
function formatMetrics(metrics) {
    ...
    const formatters = {
        height: (val) => `Height: #${val?.toLocaleString() || 'N/A'}`,
        commitment_count: (val) => `Commitments: ${val?.toLocaleString() || '0'}`,
        difficulty: (val) => `Difficulty: ${val || 'N/A'}`,
        mining_active: (val) => `Mining: ${val ? '🟢 Active' : '🔴 Inactive'}`,
        bridge_contracts: (val) => `Bridges: ${val || 0}`,
        platforms: (val) => `Platforms: ${Array.isArray(val) ? val.length : 0}`
    };

    return Object.entries(metrics)
        .map(([key, value]) => {
            const formatter = formatters[key];
            return formatter ? `<span class="metric">${formatter(value)}</span>` : '';
        })
        .filter(Boolean)
        .join(' • ');
}

function updateNetworkHealthDisplay(networkHealth) {
    // Update hero stats with real-time data
    const stats = document.querySelectorAll('.stat-value');
    if (stats.length >= 3) {
        stats[0].textContent = networkHealth.height ? `${(networkHealth.height / 1000).toFixed(1)}K` : '1.2M'; // Fallback to design value
        stats[1].textContent = networkHealth.peers_connected ? `${networkHealth.peers_connected}K+` : '50K+';
        // Keep transaction cost static for now
    }
}

function getNetworkStatus(networkHealth) {
    if (!networkHealth) return 'offline';

    const isHealthy = networkHealth.mining_active &&
                     networkHealth.peers_connected > 5 &&
                     networkHealth.height > 0;

    return isHealthy ? 'online' : 'warning';
}

function createStatusIndicator() {
    const roadmapSection = document.querySelector('#roadmap .container');
    if (!roadmapSection) return document.createElement('div');

    const statusDiv = document.createElement('div');
    statusDiv.className = 'roadmap-status';
    ...
    roadmapSection.appendChild(statusDiv);
    return statusDiv;
}
