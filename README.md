...
<p align="center">
  ...
  <img alt="Rust" src="https://img.shields.io/badge/rust-98k+_lines-orange">
...
<p align="center">
  ...
  <a href="https://medium.com/@trusts-stack-network">Medium</a> &bull;
  <a href="https://x.com/tsn_chain">X</a>
</p>
...
| **Verified Snapshots** | Decentralized snapshot system with Ed25519 signatures, multi-seed confirmation, and state root verification |
...
| **P2P version gate** | Nodes below MINIMUM_VERSION (v2.1.4) are rejected |
...
TSN includes a decentralized snapshot system for instant chain restoration. Snapshots capture the full chain state and are cryptographically verified through a multi-seed confirmation process.

### How It Works

1. **Production** — A seed node exports the current state and signs the manifest with its Ed25519 key
2. **Cross-confirmation** — At least 2 other independent seeds verify the block hash and state root, then sign their own confirmation
3. **Publication** — The snapshot and signed manifest are published to [GitHub Releases](https://github.com/trusts-stack-network/tsn-snapshots/releases) as a public distribution mirror
4. **Verification** — Any node importing a snapshot checks: producer signature, 2+ seed confirmations, SHA256 integrity, and recalculated state root

### Trust Model

GitHub is a **distribution mirror**, not the source of truth. Trust is established by:
- **Ed25519 signatures** from the producing seed
- **Independent confirmations** from multiple seed nodes
- **SHA256** hash of the compressed snapshot file
- **State root recalculation** after import

### Restore from Snapshot

```bash
# Download from GitHub
gh release download snapshot-XXXXX --repo trusts-stack-network/tsn-snapshots

# Verify and import (requires signed manifest)
./tsn restore-snapshot \
    --snapshot snapshot-XXXXX.json.gz \
    --manifest manifest-XXXXX.json \
    --data-dir ./data
...
The `restore-snapshot` command enforces all 4 verification checks before importing. Without a valid signed manifest with 2+ seed confirmations, the import is rejected.

### Retention

Snapshots follow a **rolling 24-hour retention** policy. The last available snapshot is never deleted, ensuring continuous coverage even if production pauses.

## Testnet Status

The private testnet v2 is live with **strict version enforcement** (v2.1.4 minimum).
...
- **Signed snapshot manifests** with Ed25519 producer signature and multi-seed cross-confirmation
- **`restore-snapshot` CLI command** for offline verified import from signed snapshots
- **5 new snapshot API endpoints**: `/snapshot/latest`, `/snapshot/manifest/:height`, `/snapshot/history`, `/snapshot/export`, `/snapshot/confirm`
- **Explorer overhaul**: P2P freshness tracking, miner PeerID identification, Snapshots tab, block toast notifications
- **MINIMUM_VERSION raised to 2.1.4**
- **Rolling snapshot retention** with guaranteed coverage (never deletes the last snapshot)
- **Public snapshot repository**: [tsn-snapshots](https://github.com/trusts-stack-network/tsn-snapshots)
...
- **Sync stability (patches A+B+D)**: validated across 6 stress tests with 20 concurrent miners for 1 hour
...
| `GET /snapshot/latest` | Latest signed snapshot manifest |
| `GET /snapshot/history` | Available snapshots with confirmation counts |
| `POST /snapshot/export` | Trigger snapshot export + cross-confirmation |
| `POST /snapshot/confirm` | Seed-to-seed manifest verification |

## Links

- **Website**: [tsnchain.com](https://tsnchain.com)
- **Explorer**: [explorer.tsnchain.com](https://explorer.tsnchain.com)
- **Snapshots**: [github.com/trusts-stack-network/tsn-snapshots](https://github.com/trusts-stack-network/tsn-snapshots)
