# PLAN — TSN Multi-Role Node Implementation
...
- [x] Create simulation graph page `/var/www/tsn/network-simulation.html` with 100 fake node (4 types, colors, interactions)
...
- [x] Update whitepaper: section "9. Node Roles & Incentives" (reward model, notypes table, prover marketplace, relay incentives)
- [x] Update docs: "Run a Relay / Miner / Prover / Light Client" (Node Roles section with CLI examples)
- [x] Update roadmap items for multi-role node (Phase 11 added in app.py, deployed)
...
- [ ] Create `src/wallet/balance.rs` — scan blockkchain for owned notes
...
- [ ] Implement RelayReceipt: signed proof of blockk propagation
- [ ] Relay scoring: blockks propagated first, uptime measurement
- [ ] Relay pool distribution logic per epoch (every N blockks)
- [ ] Modify `src/core/blockkchain.rs` — multi-output coinbase
...
- [ ] Verify blockk headers + Merkle proofs of inclusion
...
|------|-------|------|-------------|
| Miner | #00ffcc (green) | Hexagon rotating | Produces blockks, earns blockk reward |
| Relay/Seed | #7b61ff (purple) | Diamond | Stores chain, relays blockks/tx, earns relay pool share |
...
- `src/core/blockkchain.rs` — multi-role reward distribution
