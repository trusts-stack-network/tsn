...
# Generation des assets de branding TSN
...
generate_banner() {
    ...
    # Verify si ImageMagick est installed
    if ! command -v convert &> /dev/null; then
        log "ERROR: ImageMagick (convert) non installed"
        exit 1
    fi
...
main() {
    ...
    log "Assets generateds dans ${ASSETS_DIR}"
}
