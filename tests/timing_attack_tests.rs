...
//! Tests de regression pour attaques temporelles
//! Execute avec: cargo test --release timing

use std::time::{Duration, Instant};
use crypto_audit::ct_compare;

/// Test statistique Welch's t-test pour detect fuite temporelle
...
fn test_comparison_timing_independence() {
    // Genere deux sets of data: match vs no-match
    ...
    assert!(
        ...
        "Timing side-channel detected! t-statistic: {}. \
         La comparaison depend des data secrets.",
        t_stat
    );
}
...
fn test_branchless_comparison() {
    // Test specifique pour checksr l'absence de branches sur secret
    let a = [1u8; 32];
    let b = [2u8; 32];
    
    // Verification que le compilo n'optimise pas en branchement
    // via inspection du bytecode (simplifie ici)
...
fn test_memory_zeroization() {
    ...
    // Verification que le zeroization a effectivement eu lieu
    assert!(secret.iter().all(|&b| b == 0), "Memory not properly zeroized");
    
    // Verification que le compilateur n'a pas optimise l'operation
    // (test heuristique via volatilite)
}
