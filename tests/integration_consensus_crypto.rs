...
//! Tests d'integration consensus + crypto
//!
//! Suite de tests qui checks l'ensemble du pipeline :
//! validation de blocs → verification SLH-DSA → update du state tree Poseidon2
//!
//! THREAT MODEL:
//! - Adversaire peut forger des blocs malformeds
//! - Adversaire peut tenter des attaques par signature forgee
...
impl TestStateView {
    ...
    fn apply_transaction(&mut self, tx: &Transaction) -> Result<(), ValidationError> {
        // Simulation simple : throughput/credit des comptes
    ...
    fn compute_state_root(&self, _txs: &[Transaction]) -> Result<Hash, ValidationError> {
        // Pour les tests, on retourne un hash deterministic base sur l'state
...
/// Generateur de blocs valides pour les tests
...
fn test_valid_block_validation_flow() {
    ...
    // Calculer le state root after application de la transaction
...
fn test_invalid_signature_rejection() {
    ...
    // Create a bloc avec signature corrompue
...
fn test_state_root_mismatch_rejection() {
    ...
    // Create a bloc avec un mauvais state root
    let mut block = generator.create_valid_block(
        vec![tx],
        Hash::zero(),
...
proptest! {
    ...
    fn prop_state_tree_consistency(
        ...
        for (key, value) in operations {
            ...
            // Check that les valeurs inserees sont recuperables
            prop_assert_eq!(
                state_tree.get(&key),
    ...
    fn prop_block_validation_rejects_malformed_blocks(
        ...
        // Corrompre le bloc de maniere random
        ...
        if !block_bytes.is_empty() && corruption_index < block_bytes.len() {
            ...
            // Try to deserialiser et valider le bloc corrompu
            if let Ok(corrupted_block) = Block::deserialize(&block_bytes) {
                // Si la deserialization reussit, la validation doit fail
...
mod regression_tests {
    ...
    fn test_regression_double_spend_prevention() {
        // Regression : s'assurer qu'un double spend est detecte
        let validator = Validator::new();
        let generator = TestBlockGenerator::new();
        let mut state_view = TestStateView::new();
        ...
        // First transaction utilisant l'input
        ...
        // Second transaction utilisant le same input (double spend)
        ...
        // Doit be rejete (solde insuffisant after le premier spend)
        assert!(validator.validate_block(&block2, None, &state_view).is_err());
    }

    #[test]
    fn test_regression_signature_malleability() {
        // Regression : s'assurer qu'une signature ne peut pas be modifiee
        let validator = Validator::new();
        let generator = TestBlockGenerator::new();
        let state_view = TestStateView::new();
        ...
        // Try to modifier la signature (attaque de malleabilite)
...
/// Tests de performance et resistance aux attaques DoS
mod performance_tests {
    ...
    fn test_validation_performance_large_block() {
        ...
        // Create a bloc avec beaucoup de transactions
        ...
        // La validation doit reussir et be raisonnablement rapide
        assert!(result.is_ok());
    ...
    fn test_constant_time_signature_verification() {
        ...
        // Tester avec differentes tailles de messages
        ...
        // Check that les temps ne varient pas trop (resistance aux timing attacks)
