...
fn test_signature_verification_timing() {
    ...
    // Mesurer temps verification signature valide
    ...
    // Mesurer temps verification signature invalid
    ...
    // Difference doit be < 5% (contre les timing attacks)
    ...
    assert!(diff < threshold, 
        "Difference de timing detectee: {:?} > {:?}", diff, threshold);
}
...
fn test_key_comparison_constant_time() {
    ...
    // Doit be constant-time
    let diff = (same_avg.as_nanos() as i64 - diff_avg.as_nanos() as i64).abs();
    assert!(diff < 100, "Comparaison non constant-time: {}ns de difference", diff);
}
