...
fn test_mldsa65_wycheproof_vectors() {
    ...
    for group in test_set.groups {
        for test in group.tests {
            ...
            match test.result {
                TestFlag::Valid => assert!(result, "Signature valide rejetee"),
                TestFlag::Invalid => assert!(!result, "Signature invalid acceptee"),
                TestFlag::Acceptable => {
                    // Acceptable signifie qu'on peut accepter ou rejeter
                    // On doit be constant-time peu importe
...
fn test_mldsa65_fault_injection() {
    // Test la resistance aux fault injections
    ...
    let corrupted_sig = MLDSA65Signature::from_bytes(&corrupted_sig)
        .expect("Signature corrompue invalid");
