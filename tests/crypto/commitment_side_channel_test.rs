...
fn test_commitment_side_channel_resistance() {
    ...
    // Genere deux valeurs proches pour tester la fuite d'information
    let value1 = 1000u64;
    let value2 = 1001u64;
    ...
    // Checks that les commitments sont indiscernables
    assert_ne!(commit1, commit2, "Different values should produce different commitments");
    
    // Test la property d'hiding - same valeur avec blinding different
...
fn test_commitment_blinding_entropy() {
    // Checks that le blinding a suffisamment d'entropie
