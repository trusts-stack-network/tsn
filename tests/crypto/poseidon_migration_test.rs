...
//!
//! Tests de non-regression pour valider la transition cryptographique
//! entre les deux versions de Poseidon utilisees dans TSN.
...
fn test_cache_functionality() {
    ...
    // Check that le cache contient maintenant l'entree
    let (cache_size, _) = migrator.cache_stats().unwrap();
    assert_eq!(cache_size, 1);
    
    // Second hash - devrait usesr le cache
    let hash2 = migrator.hash(&input).unwrap();
    assert_eq!(hash1, hash2);
...
fn test_cache_eviction() {
    let config = MigrationConfig {
        ...
        strict_validation: false,
        enable_cache: true,
        max_cache_size: 2, // Cache very petit pour tester l'eviction
    };
    let mut migrator = PoseidonMigrator::with_config(config);
    
    // Remplir le cache au-dela de sa capacite
    for i in 0..5 {
        let input = vec![i, i + 1];
...
fn test_invalid_input_length() {
    ...
    // Test avec une entree de taille incorrecte (si V1 a des contraintes)
    let invalid_input = vec![1, 2, 3]; // Supposons que V1 n'accepte que 2 elements
    let result = migrator.hash(&invalid_input);
    
    // Le result depend de l'implementation de V1
    // Si V1 a des contraintes de taille, cela devrait fail
    match result {
        Ok(_) => {
            // V1 accepte cette taille d'entree
        }
        Err(MigrationError::InvalidInputLength { .. }) => {
            // V1 rejette cette taille d'entree - comportement attendu
        }
        Err(e) => {
...
fn test_consistency_across_modes() {
    ...
    // Hash en mode compatibility
    migrator.set_mode(PoseidonMode::Compatibility);
    let compat_hash = migrator.hash(&input).unwrap();
    
    // Le mode compatibility devrait retourner le same result que V2
    assert_eq!(v2_hash, compat_hash, "Compatibility mode should return V2 result");
}

// Tests bases sur les propertys avec proptest
proptest! {
    #[test]
...
fn test_migration_validation() {
    ...
    // Create vecteurs de test factices
    ...
    // Note: Ce test faila probablement car nous n'avons pas de vrais vecteurs de test V1
    // mais il demontre la structure de test de migration
    let result = migrator.test_migration(&test_vectors);
    
    // Pour l'instant, on accepte que le test puisse fail
    // car nous n'avons pas de vrais vecteurs de test V1
    match result {
        Ok(_) => {
            // Migration validee successfully
        }
...
fn test_strict_validation_config() {
    ...
    // Les deux devraient fonctionner, mais le strict peut be plus lent
    ...
    // Les results devraient be identiques (same algorithme)
...
mod benchmarks {
    ...
    fn benchmark_cache_impact() {
        ...
        // Le cache devrait be plus rapide pour les acces repetes
