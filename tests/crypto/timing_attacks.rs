...
fn test_signature_comparison_timing() {
    ...
    // Create signatures avec des differences bit par bit
    ...
    // Check that les temps de comparaison sont similaires (±10%)
...
fn test_nullifier_comparison_constant_time() {
    ...
    // La variance devrait be faible si l'operation est constant-time
    assert!(
...
fn test_secret_key_zeroization() {
    // Check that les keys secrets sont correctement effacees de la memory
    let secret = SecretKey::generate(&mut ChaCha20Rng::from_entropy());
    let secret_bytes = secret.to_bytes();
    
    // Drop explicit de la key
    drop(secret);
    
    // Check that la memory a ete effacee (dans la mesure du possible)
    // Note: Ce test est limite par les garanties Rust, mais checks la presence
    // d'une methode zeroize() si implementee
