...
//! Tests de security pour auditer et demontrer les vulnerabilitys crypto
//! Ces tests DOIVENT fail avec le code vulnerable current
...
fn test_timing_attack_vulnerability() {
    ...
    // VULNERABILITY: Le timing devrait be different
    // (ce test fails avec l'implementation securisee)
    println!("Timing wrong first byte: {:?}", duration_wrong_first);
    println!("Timing wrong last byte: {:?}", duration_wrong_last);
    
    // Avec l'implementation vulnerable, wrong_first est plus rapide
    assert!(duration_wrong_first < duration_wrong_last);
}

/// Test de regression: Nonce reuse catastrophique
#[test]
fn test_nonce_reuse_vulnerability() {
    ...
    // Reset counter pour reusesr le same keystream
    cipher.counter = 0;
    let ciphertext2 = cipher.encrypt(plaintext2);
    
    // VULNERABILITY: XOR des ciphertexts revele XOR des plaintexts
...
/// Test de regression: RNG previsible
#[test]
fn test_predictable_rng_vulnerability() {
    ...
    // VULNERABILITY: Same seed = same output
    assert_eq!(buf1, buf2);
    
    // Test de predictibilite
...
/// Test de regression: Padding oracle
#[test]
fn test_padding_oracle_vulnerability() {
    // Data avec padding PKCS#7 valide
    ...
    // VULNERABILITY: Timing different selon la validite
    println!("Valid padding timing: {:?}", duration_valid);
    println!("Invalid padding timing: {:?}", duration_invalid);
    
    // L'implementation vulnerable a des timings differents
    // (ce test peut be flaky selon la charge system)
}

/// Test de regression: KDF faible
#[test]
fn test_weak_kdf_vulnerability() {
    let password = b"password123";
    
    // VULNERABILITY: Same password = same hash (pas de sel)
    let hash1 = naive_kdf(password);
    let hash2 = naive_kdf(password);
    
    assert_eq!(hash1, hash2);
    
    // VULNERABILITY: Hashes previsibles pour mots de passe communs
    ...
    // Ces hashes peuvent be precalcules (rainbow table)
...
proptest! {
    ...
    fn prop_mac_timing_attack(
        ...
        // Mesurer timing pour differentes positions d'error
        ...
        if mac1.len() > 1 {
            ...
            // VULNERABILITY: Timing leak detectable
            if duration_first.as_nanos() > 0 && duration_last.as_nanos() > 0 {
                ...
                // Avec l'implementation vulnerable, ratio > 1
    ...
    fn prop_static_nonce_catastrophe(
        ...
        // VULNERABILITY: Nonce statique = ciphertexts lies
        // (En AES-GCM, c'est catastrophique - revele la key)
        prop_assert_ne!(ciphertext1, ciphertext2);
        
        // Test additionnel: same plaintext = same ciphertext (catastrophique)
...
/// Test d'integration: Exploitation complete d'un timing attack
#[test]
fn test_completee_timing_attack_exploitation() {
    ...
    for pos in 0..secret_token.len() {
        ...
        for candidate in 0..=255u8 {
            ...
            // Le byte correct prend plus de temps (va plus loin dans la loop)
    ...
    // VULNERABILITY: Token completement devine via timing
    assert_eq!(&guessed_token, secret_token);
}

/// Test de regression: Padding oracle differentiation
#[test]
fn test_padding_oracle_error_differentiation() {
    let key = b"test_key_16_byte";
    
    // Differents types d'errors de padding
    ...
    // VULNERABILITY: Erreurs differentiables = oracle
    match (result1, result2, result3) {
        (
            ...
            // L'attaquant peut distinguer les types d'errors
            assert!(true);
        }
...
mod benchmarks {
    ...
    /// Benchmark pour detect les timing leaks
    #[test]
    fn benchmark_timing_attacks() {
        ...
        // VULNERABILITY detectable: ratio significativement > 1
