...
//!
//! Tests de performance specifiques au gossip protocol:
//! - Latence de propagation des blocs avec 100+ peers
//! - Bande passante utilisee sous charge
//! - Resistance aux pics de trafic
//! - Degradation gracieuse sous stress
...
/// Metrics de performance pour les tests de charge
#[derive(Debug, Clone)]
...
/// Noeud de test simule pour les tests de charge
#[derive(Debug)]
...
impl LoadTestNode {
    ...
    /// Creates a node malveillant pour les tests adversariaux
    ...
    pub async fn send_gossip(&self, message: GossipMessage, targets: &[SocketAddr]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for &target in targets {
            // Simule la latence network
            let network_delay = if self.is_malicious {
                Duration::from_millis(0) // Noeud malveillant sans delai
            } else {
    ...
    /// Simule la reception d'un message
    ...
    /// Genere du spam si le node est malveillant
    pub async fn spam_network(&self, targets: &[SocketAddr], spam_count: u32) {
        ...
        for i in 0..spam_count {
            ...
            // Petit delai pour avoid de saturer completement
            if i % 10 == 0 {
...
/// Network de test pour les tests de charge
...
impl LoadTestNetwork {
    ...
    /// Creates a topologie en etoile (un node central connecte a tous)
    pub fn create_star_topology(&mut self, center_node: u32) {
        self.topology.clear();
        
        for node in &self.nodes {
            if node.id == center_node {
                // Le node central est connecte a tous
                ...
                // Les autres nodes ne sont connectes qu'au centre
                self.topology.insert(node.id, vec![center_node]);
            }
        }
    }

    /// Creates a topologie en grille (chaque node connecte a ses voisins)
    ...
    /// Simule la propagation d'un bloc a travers le network
    pub async fn propagate_block(&self, origin_node: u32, block: Block) -> Duration {
        ...
        // Trouve le node d'origine
        let origin = self.nodes.iter().find(|n| n.id == origin_node).unwrap();
        
        // Gets thes connections du node d'origine
        if let Some(connections) = self.topology.get(&origin_node) {
            ...
            // Envoie le message aux peers connectes
    ...
    async fn simulate_cascade_propagation(&self, message: GossipMessage, from_nodes: &[u32], hop_count: u8) {
        if hop_count > 10 { // Limite la profondeur pour avoid les loops
            return;
        }
        ...
        for &node_id in from_nodes {
            if let Some(connections) = self.topology.get(&node_id) {
                ...
                // Recursion pour la prochaine couche
                tokio::spawn({
                    let network = self;
                    ...
                    let hc = hop_count + 1;
                    async move {
                        // Note: dans un vrai test, on passerait une reference
                        // Ici on simule juste le delai
                        sleep(Duration::from_millis(1)).await;
                    }
                });
            }
    ...
    /// Collecte les metrics de tous les nodes
    ...
    /// Ajoute des nodes malveillants au network
...
async fn test_gossip_latency_100_peers() {
    ...
    network.create_mesh_topology(10); // Chaque node connecte a 10 autres
    ...
    // Propage le bloc depuis le node 0
...
async fn test_gossip_bandwidth_under_load() {
    ...
    for i in 1..=100 {
        ...
        // Petit delai entre les blocs
        sleep(Duration::from_millis(10)).await;
    }

    // Attend que tous les messages soient traites
    sleep(Duration::from_secs(2)).await;
    
    let metrics = network.collect_metrics().await;
    let test_duration = start_time.elapsed();

    info!("=== RESULTS TEST BANDE PASSANTE ===");
    info!("Duration du test: {:?}", test_duration);
    info!("Data transmises: {} bytes", metrics.bandwidth_bytes);
    info!("Debit: {:.2} Mbps", metrics.throughput_mbps(test_duration));
    info!("Messages/seconde: {:.2}", metrics.messages_sent as f64 / test_duration.as_secs_f64());

    // Assertions de performance
    assert!(metrics.bandwidth_bytes > 0, "Aucune donnee transmise");
    assert!(metrics.throughput_mbps(test_duration) > 0.1, "Debit trop faible");
}

#[tokio::test]
async fn test_gossip_resilience_with_malicious_nodes() {
    let mut network = LoadTestNetwork::new(80, 9200).await;
    network.add_malicious_nodes(20, 9300).await; // 20% de nodes malveillants
    network.create_mesh_topology(15);

    info!("Starting resilience test with 20% malicious nodes");
    let start_time = Instant::now();

    // Les nodes malveillants commencent a spammer
    ...
    // Pendant ce temps, propage des blocs legitimes
    for i in 1..=20 {
    ...
    info!("=== RESULTS TEST RESILIENCE ===");
    info!("Messages legitimes vs spam ratio: {:.2}", 
          metrics.messages_received as f64 / metrics.messages_sent as f64);
    info!("Failures de livraison: {}", metrics.failed_deliveries);
    info!("Debit maintenu: {:.2} Mbps", metrics.throughput_mbps(test_duration));

    // Le network doit maintenir des performances acceptables malgre l'attaque
    assert!(metrics.avg_latency() < Duration::from_millis(500), 
            "Latence degradee par l'attaque: {:?}", metrics.avg_latency());
}

#[tokio::test]
async fn test_gossip_scalability_200_peers() {
    let mut network = LoadTestNetwork::new(200, 9400).await;
    network.create_mesh_topology(20); // Chaque node connecte a 20 autres

    info!("Starting scalability test with 200 peers");
    let start_time = Instant::now();

    // Test de montee en charge progressive
    for batch in 1..=5 {
    ...
    info!("=== RESULTS TEST SCALABILITY 200 PEERS ===");
    info!("Duration totale: {:?}", test_duration);
    info!("Latence moyenne: {:?}", metrics.avg_latency());
    info!("Latence P95: {:?}", metrics.p95_latency());
    info!("Debit global: {:.2} Mbps", metrics.throughput_mbps(test_duration));
    info!("Efficacite network: {:.2}%", 
          (metrics.messages_received as f64 / metrics.messages_sent as f64) * 100.0);

    // Assertions de scalabilite
    ...
    assert!(metrics.throughput_mbps(test_duration) > 1.0, 
            "Debit insuffisant pour 200 peers");
}
...
async fn test_gossip_memory_usage_under_stress() {
    ...
    // Generates ae charge soutenue pendant 30 secondes
    ...
    while Instant::now() < end_time {
        ...
        // Frequence elevee: un bloc toutes les 100ms
        sleep(Duration::from_millis(100)).await;
    }
    ...
    info!("=== RESULTS TEST STRESS MEMORY ===");
    info!("Blocs generes: {}", block_counter);
    info!("Duration reelle: {:?}", actual_duration);
    info!("Messages traites: {}", metrics.messages_received);
    info!("Debit soutenu: {:.2} Mbps", metrics.throughput_mbps(actual_duration));
    info!("Latence sous stress: {:?}", metrics.avg_latency());

    // Le system doit maintenir des performances stables sous stress
    assert!(metrics.avg_latency() < Duration::from_millis(400), 
            "Latence degradee sous stress: {:?}", metrics.avg_latency());
    assert!(block_counter > 250, "Pas assez de blocs generes: {}", block_counter);
}
