...
//! Checks that les operations secrets ne fuient pas via timing.
...
fn secret_key_constant_time_eq() {
    ...
    // Methode constant-time
    let ct_eq = sk1.as_bytes().ct_eq(sk2.as_bytes()).unwrap_u8();
    // Methode non constant-time (doit be deletede after refactor)
    let rust_eq = sk1 == sk2;
    // On s'assure qu'on peut compiler les deux mais qu'on usesra
...
proptest::proptest! {
    ...
    fn poseidon_hash_different_inputs_different_outputs(a: Vec<u8>, b: Vec<u8>) {
        // Invariant : deux entrees differentes ⇒ hash different
        prop_assume!(a != b);
