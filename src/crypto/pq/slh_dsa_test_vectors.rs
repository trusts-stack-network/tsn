...
//!
//! Ces vecteurs sont generateds selon les specifications FIPS 205 pour les parameters
//! SHA2-128s (n=16, h=63, d=7, a=12, k=14, w=16)

/// Seed de test pour generation deterministic (32 bytes)
/// Used pour reproduire les same keys dans les tests
...
/// Message avec characters special et UTF-8
pub const MESSAGE_UTF8: &[u8] = "Test avec characters special: eaucn 日本語 🎉".as_bytes();

/// Message avec bytes nuls et valeurs extreme
...
pub const EXPECTED_PK_128S: &[u8] = &[
    // Ces valeurs seraient generatedes par un outil de reference FIPS 205
    // Pour les tests, on utilise des placeholders qui seront replaced
...
/// Note: En pratique, on ne stocke pas la signature completee ici
/// mais on verifies les properties (taille, format)
...
/// Data pour tests de malleability
/// Une signature modified qui ne should pas be valid
pub const MALLEABLE_SIG_HEADER: &[u8] = &[
    0x00, 0x00, 0x00, 0x00, // Version incorrecte
];

/// Data pour tests de corruption
/// Indices d'bytes critiques dans la signature qui ne doivent pas be modifieds
pub const CRITICAL_SIG_OFFSETS: &[usize] = &[0, 1, 2, 3, 4, 5, 6, 7];

/// Nombre de signatures to generate pour tests de non-collision
pub const NON_COLLISION_ITERATIONS: usize = 100;

/// Messages pour tests de domaine separated
...
/// Structure pour les cas de test named
pub struct TestCase {
    pub name: &'static str,
    pub seed: &'static [u8],
...
/// Vecteurs de test pour verification d'erreur
...
/// Cas d'erreur pour tests de verification
