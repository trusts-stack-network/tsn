...
//!
//! Implementation de Poseidon2 selon le papier « Poseidon2: A Faster Version of the Poseidon Hash Function »
//! (Grassi et al., 2023). Parameters choisis pour un niveau de security de 128 bits contre les attaques
//! classiques et quantiques, avec un taux de resistance aux preimages de 2^128.
//!
//! Security :
...
/// Parameters de security pour Poseidon2
#[derive(Debug, Clone)]
...
impl Default for Poseidon2Params {
    fn default() -> Self {
        // Parameters pour 128 bits de security sur BN254
        Self::new(3, 5, 8, 56)
    }
}
...
impl Poseidon2Params {
    ...
    pub fn new(t: usize, d: u64, rounds_f: usize, rounds_p: usize) -> Self {
        ...
        // Generation des round keys (doit be deterministic)
        params.generate_round_keys();
        
        // Generation de la MDS matrix
    ...
    fn generate_round_keys(&mut self) {
        ...
        for i in 0..total_rounds {
            for j in 0..self.t {
                // Generation pseudo-random mais deterministic
...
/// State interne du hash Poseidon2
#[derive(Debug, Clone)]
