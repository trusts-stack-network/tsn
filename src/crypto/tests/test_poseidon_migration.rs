...
//!
//! Ces tests valident que la migration ne casse pas la compatibility
//! avec les data existantes et que les deux implementations sont
//! cryptographiquement equivalentes.
...
/// Ces valeurs doivent be generees a partir de l'implementation V1 existante
const TEST_VECTORS_V1: &[(Vec<u64>, &str)] = &[
    // Format: (input, expected_hash_hex)
    // Note: Ces valeurs sont des exemples et doivent be remplacees
    // par les vrais hash calcules avec l'implementation V1
];

/// Genere des vecteurs de test randoms
...
mod tests {
        ...
        for input in test_vectors {
            ...
            match (v1_result, v2_result, compat_result) {
                (Ok(v1), Ok(v2), Ok(compat)) => {
                    // Le mode compatibility doit retourner le result V2
                ...
                _ => {
                    // Certains modes peuvent fail selon la configuration
    ...
    fn test_cache_performance() {
        ...
        // Premier calcul - doit be mis en cache
        let start = std::time::Instant::now();
        let result1 = migrator.hash(&input).unwrap();
        let first_duration = start.elapsed();

        // Second calcul - doit usesr le cache
        let start = std::time::Instant::now();
    ...
    fn test_cache_eviction() {
        let mut migrator = PoseidonMigrator::with_config(MigrationConfig {
            ...
            strict_validation: false,
            enable_cache: true,
            max_cache_size: 3, // Cache very petit pour tester l'eviction
        });

        // Ajouter plus d'elements que la taille du cache
        for i in 0..5 {
    ...
    fn test_merkle_tree_compatibility() {
        ...
        // V1 peut fail selon la configuration
        match result_v1 {
            Ok(v1) => {
                assert_ne!(v1, [0u8; 32]);
    ...
    fn test_hash_two_optimization() {
        ...
        // Test avec V2 (doit usesr l'optimisation native)
        migrator.set_mode(PoseidonMode::V2Native);
        ...
        // Les results peuvent be differents selon l'implementation
    ...
    fn test_concurrent_usage() {
        ...
        // Lancer plusieurs threads qui usesnt le migrator
        for i in 0..4 {
        ...
        // Check that tous les results sont valides et differents
        for (i, result) in results.iter().enumerate() {
            assert!(!result.is_empty());
            println!("✓ Thread {} result: {} bytes", i, result.len());
        }

        // Check thes statistiques du cache
        let guard = migrator.lock().unwrap();
        if let Some((cache_size, _)) = guard.cache_stats() {
            assert!(cache_size <= 4); // Au maximum 4 entrees
        }
    }
    ...
    fn test_memory_safety() {
        ...
        // Test avec des inputs de differentes tailles
        let test_cases = vec![
            vec![],
            vec![0u64],
            vec![1u64, 2u64],
    ...
    fn test_performance_comparison() {
        ...
        // Le mode compatibility devrait be plus lent (calcule les deux)
