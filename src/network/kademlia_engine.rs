...
//! 
//! Implements le core du system DHT avec lookup iterative, bootstrap,
//! stockage local et maintenance des peers. Designed pour la robustesse
...
/// Request en attente de response
#[derive(Debug)]
...
impl KademliaEngine {
    ...
    pub async fn start(&self) -> Result<(), DhtError> {
        ...
        // Starts la maintenance periodic
        let engine = self.clone();
        tokio::spawn(async move {
    ...
    /// Lookup iterative pour FIND_NODE
    pub async fn iterative_find_node(&self, target: NodeId) -> Result<Vec<KademliaNode>, DhtError> {
        ...
        // Trouve les nodes de start depuis la table de routage
        let initial_nodes = {
        ...
        loop {
            // Selectionne les prochains nodes to interroger
            ...
            // Sendinge les requests en parallel (limited par ALPHA)
    ...
    /// Interroge un node specific pour une cible
    ...
    pub async fn store_value(&self, key: DhtKey, value: Vec<u8>) -> Result<(), DhtError> {
        ...
        // Replicates sur les K nodes les plus proches
        ...
        // Attend que au moins la majority succeeds
    ...
    /// Stocke une valeur sur un node specific
    ...
    pub async fn find_value(&self, key: DhtKey) -> Result<Option<DhtValue>, DhtError> {
        // Verifies d'abord le stockage local
        {
        ...
        // Lookup iterative pour FIND_VALUE
        let key_node_id = NodeId::new(key);
        let closest_nodes = self.iterative_find_node(key_node_id).await?;
    ...
    /// Recherche une valeur sur un node specific
    ...
    /// Sendinge un message to un node (simulation)
    ...
    async fn maintenance_loop(&self) {
        ...
        loop {
            ...
            // Nettoie les valeurs expireds
            {
                let mut storage = self.local_storage.write().await;
                storage.retain(|_, value| !value.is_expired());
            }
            
            // Nettoie les requests timeout
            {
                let mut pending = self.pending_requests.write().await;
