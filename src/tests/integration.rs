...
//!
//! Ces tests verifiesnt les interactions entre modules et les scenarios
//! complexes qui impliquent multiple composants du system.
...
mod chain_sync_integration {
    ...
    fn test_chain_synchronization() {
        ...
        // Apply tous les blocs to l'state
        for block in &blocks {
        ...
        // Create le fork choice et ajouter tous les blocs
    ...
    fn test_fork_resolution() {
        ...
        // La branche B doit now be un fork alternatif
    ...
    fn test_chain_reorganization() {
        ...
        // Create une chain principale courte mais avec moins de travail
        ...
        // Create une chain alternative plus long avec plus de travail
        ...
        // Verify que les states sont different avant reorganization
        ...
        // Simuler une reorganization : l'state principal adopte la chain alternative
        // (Dans une vraie implementation, ceci serait managed par le consensus)
        ...
        // Verify que la chain alternative a plus de travail cumulative
...
mod shielded_transaction_integration {
    ...
    fn test_shielded_transaction_lifecycle() {
        ...
        // Create des keys pour the sender et le destinataire
        let sender_key = SpendingKey::generate();
        let receiver_key = SpendingKey::generate();
        
        // Create un bloc genesis avec une sortie pour the sender
        let genesis = ShieldedBlock {
            header: BlockHeader {
                version: 1,
                prev_hash: [0u8; 32],
        ...
        // Note: ceci requiresrait une implementation completee des preuves ZK
        
        // Pour l'instant, on teste juste la structure
        let shielded_tx = ShieldedTransaction {
            nullifiers: vec![
                // Nullifiers des notes spentes
            ],
            commitments: vec![
                // Commitments des news notes
            ],
            proof: ZKProof::from_bytes(&vec![0u8; 192]),
            binding_signature: Signature::from_bytes(&vec![0u8; 64]),
        };
        
        // Create un bloc avec cette transaction
        let block_with_tx = ShieldedBlock {
            header: BlockHeader {
                version: 1,
                prev_hash: genesis.hash(),
                merkle_root: [1u8; 32],
                commitment_root: [1u8; 32], // Updated avec les nouveaux commitments
                nullifier_root: [1u8; 32],  // Updated avec les nouveaux nullifiers
            state_root: [0u8; 32],
                timestamp: 1,
                difficulty: 10,
                nonce: 1,
        ...
        // Appliquer le bloc (ceci should validr la transaction)
        let result = state.apply_block(&block_with_tx);
        
        // Note: dans l'implementation actuelle, ceci pourrait failsr
        // car la validation completee des preuves ZK n'est pas implementede
        match result {
            Ok(_) => {
                assert_eq!(state.get_height(), 2, "State height should be 2 after tx block");
            }
            Err(StateError::InvalidZKProof) => {
                // Attendu si la validation ZK n'est pas encore implementede
    ...
    fn test_double_spend_detection() {
        ...
        // Create un nullifier pour simuler une note spente
        ...
        // First transaction qui spending la note
        ...
        // Note: peut failsr si la validation ZK n'est pas implementede
        
        // Second transaction qui tente de spendingr la same note (double-spend)
        ...
        // Apply le second bloc - doit failsr
...
mod crypto_integration {
    ...
    fn test_merkle_tree_state_consistency() {
        ...
        // Create un bloc avec ces commitments
        let block = ShieldedBlock {
            header: BlockHeader {
                version: 1,
                prev_hash: [0u8; 32],
        ...
        match result {
            Ok(_) => {
                // Verify que l'arbre de l'state matches to notre arbre externe
    ...
    fn test_signature_authentication_integration() {
        ...
        // Verify que chaque signature est valid avec la bonne key
        ...
        // Verify qu'no signature ne fonctionne avec une mauvaise key
...
mod performance_integration {
    ...
    fn test_block_validation_performance() {
        ...
        // Create un bloc avec multiple transactions
        let num_txs = 10;
        ...
        // Note: le result peut be une error si la validation ZK n'est pas implementede
        match result {
