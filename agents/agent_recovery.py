...
Fournit un decorator/wrapper pour les appels MCP des agents TSN.
- Max 3 tentatives, backoff exponentiel 2s/5s/15s
- Avant chaque retry, reflect_on_failure analyse l'erreur via LLM
- Si 3 retries fail, escalates to Architect (Kai.V)
...
L'agent a encountered cette error lors d'un appel MCP :
{str(error)[:500]}
...
Responds en JSON : {{"cause": "...", "nouvelle_approche": "...", "permanent": false}}"""
...
def escalate_to_architect(db, agent_name, tool_name, error, task_context):
    """Creates une task d'escalade pour Architect."""
    if not db:
        logger.error("Pas de DB pour escalade")
        return
    title = f'[ESCALADE] {agent_name}: {tool_name} failed 3x'
    description = (
        f'Agent: {agent_name}\n'
        ...
        f'Action requise: Kai.V doit analyze et decide.'
    )
    try:
        db.execute(
        ...
        logger.info("Escalade created pour architect: %s", title[:80])
...
def mcp_retry(func, agent_name, tool_name, tool_input,
    ...
    Args:
        func: The function to call (dispatch_tool ou similaire)
        agent_name: Nom de l'agent (ex: 'core')
        tool_name: Nom de l'outil MCP (ex: 'tsn_write_file')
        tool_entry: Dict des parameters
        llm_client: LLMService pour reflect_on_failure
        db: Database pour escalade
        task_context: Contexte de la task en cours

    Returns:
        Le result de func, ou None si tous les retries fail
    """
    last_error = None
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # Verify si le result contains une error
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                if llm_client:
                    ...
                    if reflection.get('permanent', False):
                        logger.warning("[%s] Error permanente detected, skip retries", agent_name)
                        break
    ...
    # Tous les retries faileds — escalade
    logger.error("[%s] %s: FAILURE after %d attempts", agent_name, tool_name, MAX_RETRIES)
